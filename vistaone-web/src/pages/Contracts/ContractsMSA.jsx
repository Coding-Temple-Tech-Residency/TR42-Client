...
import { msaContracts, allowedFileTypes, maxFileSizeMB } from '../../data/contractData'
...
const ContractsMSA = () => {
    ...
    const resetUpload = () => {
        setUploadFile(null); setUploadError(''); setUploadSuccess(false)
    ...
    const handleUploadSubmit = (e) => {
        ...
        // this will POST to the API as multipart/form-data once endpoints are ready
        // the file is NOT stored client-side — it will be sent directly to the server
            ...
            {showUpload && (
                <div className="contracts-upload-panel mb-4">
                    ...
                    <p className="contracts-upload-note mb-3">
                        PDF files only, max {maxFileSizeMB}MB. Files are sent securely to the server and are not stored in your browser.
                    </p>
                    ...
                    <form onSubmit={handleUploadSubmit}>
                        ...
                        <div className="d-flex gap-3">
                            ...
                            <button type="submit" className="contracts-btn-primary">Upload document</button>
                        </div>
                    </form>
                </div>
            )}
                                    ...
                                    <td>
                                        {msa.file_name ? (
                                            <div className="d-flex align-items-center gap-1">
                                                ...
                                                <span className="contracts-file-link">{msa.file_name}</span>
                                            </div>
                                        ) : (
                                            <span className="contracts-no-file">No document</span>
                                        )}
                                    </td>
                                    <td>{msa.uploaded_by || '—'}</td>
                                </tr>
                            )
                        })}
                    </tbody>
