...
const VendorDirectory = () => {
    ...
    // vendors and services now come from the API instead of the mock data file
    ...
    const filteredVendors = vendors
        ...
        .sort((a, b) => {
            ...
            if (sortBy === 'completed') return (b.completed_work_orders || 0) - (a.completed_work_orders || 0)
    ...
    const serviceCounts = serviceOptions.map((svc) => ({
        ...
        count: vendors.filter(
            (v) => v.status === 'active' && (v.services || []).some((s) => s.service_id === svc.service_id)
        ).length,
    }))
    ...
    if (loading) {
        return (
            <div className="vendor-directory">
                <p className="marketplace-results-count">Loading vendors...</p>
            </div>
        )
    }
        ...
        <div className="vendor-directory">
            ...
            <div className="vendor-filters d-flex flex-wrap gap-3 mb-4">
                ...
                <div className="vendor-filter-group d-flex align-items-center gap-2">
                    ...
                    <select className="vendor-select" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
                        <option value="all">All statuses</option>
                        <option value="active">Active</option>
                        <option value="inactive">Inactive</option>
                    </select>
                    <select className="vendor-select" value={complianceFilter} onChange={(e) => setComplianceFilter(e.target.value)}>
                        <option value="all">All compliance</option>
                        <option value="complete">Complete</option>
                        <option value="incomplete">Incomplete</option>
                        <option value="expired">Expired</option>
                    </select>
                    <select className="vendor-select" value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
                    ...
                    >
                        ...
                        <div className="marketplace-card-stats d-flex gap-4">
                            <div className="d-flex align-items-center gap-1">
                                ...
                                <span className="marketplace-stat-value fw-semibold">
                                    {vendor.average_rating != null ? vendor.average_rating : 'N/A'}
                                </span>
                                <span className="marketplace-stat-label">rating</span>
                            </div>
                            <div className="d-flex align-items-center gap-1">
                                <ClipboardList size={14} className="marketplace-stat-icon" />
                                <span className="marketplace-stat-value fw-semibold">{vendor.completed_work_orders || 0}</span>
                                <span className="marketplace-stat-label">completed</span>
                            </div>
                            <div className="d-flex align-items-center gap-1">
                                <Shield size={14} className="marketplace-stat-icon" />
                                <span className="marketplace-stat-label">
                                    {(vendor.insurance_policies || []).length} policy
                                </span>
                            </div>
                        </div>
